...
def prep_pos(
    ...
    if line in ["W", "X", "Z"]:
        ...
        pos_tot = np.hstack(
            [pos1, pos2, pos3]
        )  # dim = (spectral pixel, spatial pixel, 4)
    ...
    try:
        dlam = branchNode.getNode("MOMENTS.%s.DLAM" % (line)).data() / 1000  # [Ang]
    except Exception as e:
        print(
            f"Error loading DLAM for line {line}, using default value of 5nm. Error: {e}"
        )
        dlam = 5 / 1000  # 5nm in Ang, guess
    # Output
...
def get_pos_XICS(
    ...
    Z_T = pos_ave[:, 1] - np.tan(pos_ave[:, 3]) * np.sqrt(
        pos_ave[:, 0] ** 2 - pos_ave[:, 2] ** 2
    )  # [m], dim = n_ch
    ...
    xyz_srt = np.stack(
        (pos_ave[:, 0], 0 * pos_ave[:, 0], pos_ave[:, 1]), axis=1
    )  # dim = (n_ch, 3)
...
def _get_int(
    ...
    for ll in lines:
        ...
        try:
            int_nd = spec.getNode(
                r"\spectroscopy::top.hirex_sr.analysis." + ll + ":int"
            )
            vel_nd = spec.getNode(
                r"\spectroscopy::top.hirex_sr.analysis." + ll + ":vel"
            )
        except Exception as e:
            print(f"No data for line {ll}: {e}")
            continue
        # Stores data
        dout["int"][ll] = {}
...
def plot_vel(dout={}, intLim=[0, 1e3], velLim=[-1e2, 1e2]):
    ...
    fig, axs = plt.subplots(
        len(lines), 2, figsize=(6, 2 * len(lines)), sharex=True, layout="constrained"
    )
    for i, ll in enumerate(lines):
        # Intensity
        # W, Z lines have three channels, A, M lines have one channel. Plot all channels if multiple
        npix = (
            len(dout["int"][ll]["data"]) if np.ndim(dout["int"][ll]["data"]) > 1 else 1
        )
        ...
        if npix == 1:
            ...
            fig.colorbar(
                axs[i, 0].collections[0], ax=axs[i, 0], label="Intensity [arb. units]"
            )
        ...
        if npix == 1:
            axs[i, 0].set_ylabel(f"{ll} Intensity [arb. units]")
        else:
            axs[i, 0].set_ylabel(f"{ll} Pixel Channel")
        ...
        if npix == 1:
            ...
            fig.colorbar(
                axs[i, 1].collections[0], ax=axs[i, 1], label="Velocity [km/s]"
            )
    ...
    fig.savefig(
        f'../output_plots/shot_{dout["shot"]}_tht_{dout["tht"]}_int_vel.pdf',
        transparent=True,
    )
