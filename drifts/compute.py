...
def do_equilibrium_dimensionality_check(
    ...
    nt_eq = time_s.size
    assert (
        psi_n_q.shape == q_profile.shape
    ), f"Expected psi_n_q and q_profile to have the same shape, got {psi_n_q.shape} vs {q_profile.shape}"
    assert b_t_t.shape == (
        r_major_full_m.size,
        nt_eq,
    ), f"Expected b_t_t to have shape ({r_major_full_m.size}, {nt_eq}), got {b_t_t.shape}"
    assert (
        r_minor_q_m.shape == q_profile.shape
    ), f"Expected r_minor_q_m to have the same shape as q_profile, got {r_minor_q_m.shape} vs {q_profile.shape}"
    assert (
        r_out_q.shape == q_profile.shape
    ), f"Expected r_out_q to have the same shape as q_profile, got {r_out_q.shape} vs {q_profile.shape}"
    assert elongation_t.shape == (
        nt_eq,
    ), f"Expected elongation_t to have shape ({nt_eq},), got {elongation_t.shape}"
    assert (
        circumference_q_m.shape == q_profile.shape
    ), f"Expected circumference_q_m to have the same shape as q_profile, got {circumference_q_m.shape} vs {q_profile.shape}"
    assert q95.shape == (
        nt_eq,
    ), f"Expected q95 to have shape ({nt_eq},), got {q95.shape}"
...
def do_profile_dimensionality_check(t_diag, r_all, te_eV, ti_eV, ne_m3, psi_hx, zeff_t):
    ...
    assert r_all.shape == (
        nspace,
        nt_diag,
    ), f"Expected r_all to have shape ({nspace}, {nt_diag}), got {r_all.shape}"
    assert te_eV.shape == (
        nspace,
        nt_diag,
    ), f"Expected te_eV to have shape ({nspace}, {nt_diag}), got {te_eV.shape}"
    assert (
        ti_eV.shape[1] == nt_diag
    ), f"Expected ti_eV to have time shape ( {nt_diag}), got {ti_eV.shape}"
    assert ne_m3.shape == (
        nspace,
        nt_diag,
    ), f"Expected ne_m3 to have shape ({nspace}, {nt_diag}), got {ne_m3.shape}"
    # if psi_hx is not None:
    #     assert psi_hx.shape == (nspace, nt_diag), f"Expected psi_hx to have shape ({nspace}, {nt_diag}), got {psi_hx.shape}"
    assert zeff_t.shape == (
        nt_diag,
    ), f"Expected zeff_t to have shape ({nt_diag},), got {zeff_t.shape}"
...
def _interpolate_below_threshold_radial(
    ...
    if values.shape != rvals.shape:
        raise ValueError(
            f"Interpolation shape mismatch: {values.shape} vs {rvals.shape}"
        )
...
def _clean_low_te_ne_profiles(
    ...
    te_clean = _interpolate_below_threshold_radial(
        te_keV_st, r_major_st, min_value=te_floor_keV
    )
    ne_clean = _interpolate_below_threshold_radial(
        ne_m3_st, r_major_st, min_value=ne_floor_m3
    )
...
def _ellipse_circumference(
    a_minor: np.ndarray, kappa: np.ndarray | float
) -> np.ndarray:
    ...
    return (
        np.pi
        * (a + b)
        * (1.0 + (3.0 * h) / (10.0 + np.sqrt(np.maximum(4.0 - 3.0 * h, 1e-12))))
    )
...
def _load_ti_from_hirexsr(
    ...
    rho_hx = np.concatenate((rho_hx, 0.91 * np.ones((1, rho_hx.shape[1]))), axis=0)
...
def load_profiles_for_shot(
    ...
    if not fallback_ti_eV:
        try:
            ...
            print(
                f"Warning: Ti from HIREXSR unavailable, falling back to Ti=Te. Reason: {exc}"
            )
            ti_eV = te_eV
            psi_hx = None
            rho_hx = None
...
def load_equilibrium_for_shot(
    shot: int, tree: str = "efit20", diagnosticPlot=False
) -> EquilibriumData:
    """
    ...
    if f_raw.ndim == 0:
        b_t_t = (
            np.full((r_grid_full_m.size, nt_eq), float(f_raw))
            / r_grid_full_m[:, np.newaxis]
        )
    elif f_raw.ndim == 1:
        if f_raw.size == nt_eq:
            ...
            b_t_t = np.repeat(
                (f_raw / np.maximum(r_grid_full_m, 1e-8))[:, np.newaxis], nt_eq, axis=1
            )
        else:
            raise ValueError(
                f"Unexpected getF() shape {f_raw.shape} for nt={nt_eq}, nR={r_grid_full_m.size}"
            )
    elif f_raw.ndim == 2:
        if f_raw.shape == (nt_eq, r_grid_full_m.size):
            ...
            raise ValueError(
                f"Unexpected getF() 2D shape {f_raw.shape} for nt={nt_eq}, nR={r_grid_full_m.size}"
            )
    else:
    ...
    if diagnosticPlot:
        plot_magnetic_equilibrium(
            r_out_q, _compute_b_pol_midplane(eq_f, r_out_q, nt_eq), t_eq, eq_f
        )
...
def plot_magnetic_equilibrium(
    r_out_q: np.ndarray,
    b_pol_q_T: np.ndarray,
    t_eq: np.ndarray,
    eq_f: eq.CModEFIT.CModEFITTree,
):
    import matplotlib.pyplot as plt

    fig, axs = plt.subplots(1, 2, figsize=(7, 2.5), layout="constrained", sharey=True)
    ctr1 = axs[0].contourf(
        r_out_q, np.tile(t_eq, (r_out_q.shape[0], 1)), b_pol_q_T, levels=20, zorder=-5
    )
    ...
    ctr2 = axs[1].contourf(
        eq_f.getRGrid(), t_eq, (eq_f.getF() / eq_f.getRGrid()), levels=20, zorder=-5
    )
...
def _smooth_profiles_for_gradients_axis0(
    ...
    if method_l == "savgol":
        ...
        for _ in range(passes):
            out = savgol_filter(
                out, window_length=wl, polyorder=polyorder, axis=0, mode="interp"
            )
        return out
    ...
    for it in range(out.shape[1]):
        ...
        try:
            ...
            out[:, it] = _smooth_radial_profiles_axis0(
                y_col[:, np.newaxis], passes=passes
            )[:, 0]
...
def _safe_log_gradient_sorted(
    y: np.ndarray, x: np.ndarray, eps: float = 1e-30
) -> np.ndarray:
    order = np.argsort(x)
...
def _plot_compute_grid_diagnostics(
    ...
    fig, axes = plt.subplots(
        ...
        figsize=(15, 8.5),
        num=(
            f"Drift Grid Diagnostics: shot {shot}"
            if shot is not None
            else "Drift Grid Diagnostics"
        ),
    )
    ax_q = axes[0, 0]
    ax_p = axes[0, 1]
    ...
    for i_sel, t_sel in enumerate(selected_times_s):
        ...
        ax_p.plot(
            ...
            lw=2.0,
            ls="-",
            label=rf"$p_e$ extra-sm ({time_label})",
        )
        ax_p.plot(
            r_diag_sorted,
            pi_sorted / 101325,
        ...
        ax_p.plot(
            ...
            lw=2.0,
            ls=":",
            label=rf"$p_i$ extra-sm ({time_label})",
        )
        ...
        ax_ne.plot(
            ...
            lw=2.0,
            ls="-",
            label=rf"$n_e$ extra-sm ({time_label})",
        )
        ax_te.plot(
            r_diag_sorted,
            te_sorted * 1e-3,
        ...
        ax_te.plot(
            ...
            lw=1.8,
            ls="-.",
            label=rf"$T_e$ extra-sm ({time_label})",
        )
        ...
        ax_ni.plot(
            ...
            lw=2.0,
            ls="-",
            label=rf"$n_i$ extra-sm ({time_label})",
        )
        ax_ti.plot(
            r_diag_sorted,
            ti_sorted * 1e-3,
        ...
        ax_ti.plot(
            ...
            lw=1.8,
            ls="-.",
            label=rf"$T_i$ extra-sm ({time_label})",
        )
        ax_ti.plot(
            rho_ti_t_sorted,
            ti_ev_t_sorted * 1e-3,
            ...
            lw=1.4,
            ls=":",
            label=rf"$T_i,\,\rho$ ({time_label})",
        )
        ax_rminor.plot(
            psi_sorted, r_minor_sorted, color=color, lw=1.8, label=time_label
        )
...
def compute_diamagnetic_drift_frequencies(
    ...
    ne_diag_sm = (
        ...
        if smooth_profiles_before_pressure
        else np.array(ne_m3_t, dtype=float, copy=True)
    )
    te_diag_sm = (
        _smooth_profiles_for_gradients_axis0(
            te_eV_t,
        ...
        if smooth_profiles_before_pressure
        else np.array(te_eV_t, dtype=float, copy=True)
    )
    ti_diag_sm = (
        _smooth_profiles_for_gradients_axis0(
            ti_diag_eV_t,
...
def _demo_equilibrium(time_s: np.ndarray) -> EquilibriumData:
    ...
    r_major_full_m = np.linspace(
        C_MOD_R0_M - C_MOD_A_MINOR_M, C_MOD_R0_M + C_MOD_A_MINOR_M, 196
    )
    b_t_t = b_t_time[np.newaxis, :] * (
        C_MOD_R0_M / np.maximum(r_major_full_m[:, np.newaxis], 1e-6)
    )
    ...
    return EquilibriumData(
        ...
        circumference_q_m=circumference_q_m,
        q95=q95,
        b_pol_q_T=_demo_b_pol(
            r_minor_q_m, C_MOD_R0_M + r_minor_q_m, b_t_time[np.newaxis, :], q_profile
        ),
    )
