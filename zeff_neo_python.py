...
def doConsistencyCheck(zeff, times):
    ...
    if np.any(zeff < 0.0):
        raise ValueError(
            "zeff_neo returned negative Zeff values, which are unphysical."
        )
...
def _summarize_diag_bins(diag_bins):
    ...
    local_slopes = np.array(
        [d.get("local_slope", np.nan) for d in diag_bins], dtype=float
    )
    target_pos = np.array(
        [d.get("target_position", np.nan) for d in diag_bins], dtype=float
    )
...
def zeff_neo(
    ...
    for i, z_val in enumerate(zeff_grid):
        ...
        ip_neo[:, i] = np.nansum(
            j * dv / (2.0 * np.pi * np.maximum(majr, 1.0e-16)), axis=1
        )
...
def main():
    ...
    parser.add_argument("--save-plots", type=str, default="./")
